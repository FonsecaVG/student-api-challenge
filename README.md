...
- Justificativa: Após filtrar o nome para garantir que seja somente string e todas minúsculas (lower case), decidi usar função count do Python dentro do loop para contar e validar o nº de ocorrência da letra.

- **JSON e Validação:**
- Justificativa: O Flask é usado para retornar o formato JSON e aplicar a validação da nota.
O uso do jsonify garante que a API siga o padrão RESTful. A validação numérica da nota com (try/except) prova a capacidade de lidar com entradas inválidas do cliente de forma controlada, retornando mensagens de erro claras (400 Bad Request).
